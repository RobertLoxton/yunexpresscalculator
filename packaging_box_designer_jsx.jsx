...
export default function PackagingBoxDesigner() {
  ...
  const shippingPerUnitUSD = shippingUSD / parsed.qty;
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-white to-slate-50 text-slate-800">
      <div className="mx-auto max-w-7xl px-4 py-8">
              ...
              <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div className="text-sm">
                    <div><span className="text-slate-500">Dims:</span> <span className="font-semibold">{parsed.L} × {parsed.W} × {parsed.H} {dimUnit}</span></div>
                    <div><span className="text-slate-500">Volumetric:</span> <span className="font-semibold">{calc.volWeight.toFixed(2)} {weightUnit}</span> <span className="text-slate-500">(divisor {divisorNumeric})</span></div>
                    <div><span className="text-slate-500">Battery:</span> <span className="font-semibold">{battery ? 'Built-in' : 'No'}</span></div>
                  </div>

                  <div className="flex flex-wrap gap-2">
                    <button className="rounded-xl border border-slate-300 px-3 py-1.5 text-xs hover:bg-slate-50" onClick={() => applyPreset(PRESETS_CM[0])}>Reset → Small Mailer</button>
                    <button className="rounded-xl border border-slate-300 px-3 py-1.5 text-xs hover:bg-slate-50" onClick={() => { setL(+(parsed.L * 1.05).toFixed(1)); setW(+(parsed.W * 1.05).toFixed(1)); setH(+(parsed.H * 1.05).toFixed(1)); }}>+5% size</button>
                    <button className="rounded-xl border border-slate-300 px-3 py-1.5 text-xs hover:bg-slate-50" onClick={() => { setL(Math.max(0.1, +(parsed.L * 0.95).toFixed(1))); setW(Math.max(0.1, +(parsed.W * 0.95).toFixed(1))); setH(Math.max(0.1, +(parsed.H * 0.95).toFixed(1))); }}>−5% size</button>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-center gap-2">
                    <input placeholder="Setup name (e.g., Mailer S – 30×22×10, cm5000)" value={setupName} onChange={(e) => setSetupName(e.target.value)} className="w-80 rounded-xl border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-400" />
                    <button onClick={saveCurrent} className="rounded-xl bg-slate-900 px-3 py-2 text-sm text-white">Save setup</button>
                  </div>
                  <div className="text-xs text-slate-500">Saved locally in your browser</div>
                </div>
              </div>

              {/* Saved setups table */}
              <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="mb-3 text-sm font-medium">Saved setups</div>
                {saved.length === 0 ? (
                  <div className="text-xs text-slate-500">No saved setups yet.</div>) : (
                    ...
                    <table className="min-w-full text-left text-sm">
                      <thead className="text-xs uppercase text-slate-500">
                        <tr>
                          ...
                          <th className="p-2">Ship (CNY)</th>
                          <th className="p-2">Ship (USD)</th>
                        ...
                        {saved.map((r) => (
                          <tr key={r.id} className="border-t">
                            <td className="p-2 align-top">
                              <input value={r.name} onChange={(e) => renameRow(r.id, e.target.value)} className="w-48 rounded-lg border border-slate-300 px-2 py-1 text-xs" />
                            </td>
                            <td className="p-2 align-top text-xs">{r.L}×{r.W}×{r.H} {normalizeUnits(r.units) === 'cm' ? 'cm' : 'in'}</td>
                            <td className="p-2 align-top text-xs">{r.divisorId}</td>
                            <td className="p-2 align-top text-xs">{r.battery ? 'Built-in' : 'No'}</td>
                            <td className="p-2 align-top text-xs">{r.qty}</td>
                            <td className="p-2 align-top text-xs">{Math.round(r.derived.shippingCNY)}</td>
                            <td className="p-2 align-top text-xs">{r.derived.shippingUSD.toFixed(2)}</td>
                            <td className="p-2 align-top text-xs">{r.derived.costPerUnitUSD.toFixed(2)}</td>
                            <td className="p-2 align-top text-xs">{r.derived.totalCostUSD.toFixed(2)}</td>
                            <td className="p-2 align-top text-xs">
                              <div className="flex gap-2">
                                <button onClick={() => loadRow(r)} className="rounded-lg border border-slate-300 px-2 py-1">Load</button>
                                <button onClick={() => deleteRow(r.id)} className="rounded-lg border border-rose-300 px-2 py-1 text-rose-700">Delete</button>
                              </div>
                            </td>
                          </tr>
                        ))}
        ...
        {/* Width-sensitivity explainer (optional UI helper) */}
        <div className="mt-8 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
          <h3 className="mb-2 text-base font-semibold">Why width (and all three axes) matter for shipping</h3>
          <p className="text-sm text-slate-600">
            Dimensional weight multiplies <span className="font-medium">length × width × height</span>. Increasing any one of these by just 1 unit raises the total volume by a factor proportional to the other two.
          </p>
        </div>
...
// -----------------------------
// Inline Unit Tests (dev-only) – prints results to console once
// -----------------------------
function InlineUnitTests() {
  useEffect(() => {
    if (typeof window === "undefined") return;
    if (window.__PACKAGING_TESTED__) return;

    const eq = (a, b, tol = 1e-6) => Math.abs(a - b) <= tol;

    // Geometry / volume tests (unchanged)
    let r = useBoxCalculations({ L: 30, W: 22, H: 10, units: "cm", boardMM: 2, divisor: 5000, actualWeight: 0 });
    console.assert(r.volume === 6600, "T1 volume 30×22×10 should be 6600");
    console.assert(eq(r.volWeight, 1.32), "T1 volumetric should be 1.32 kg");

    console.assert(eq(r.inner.L, 29.6) && eq(r.inner.W, 21.6) && eq(r.inner.H, 9.6), "T2 inner dims after 2mm board");

    r = useBoxCalculations({ L: 12, W: 10, H: 4, units: "in", boardMM: 0, divisor: 139, actualWeight: 0 });
    console.assert(r.volume === 480, "T3 volume 12×10×4 in³ = 480");
    console.assert(eq(r.volWeight, 480 / 139), "T3 volumetric lb checks");

    r = useBoxCalculations({ L: 30, W: 22, H: 10, units: "cm", boardMM: 0, divisor: 5000, actualWeight: 2 });
    console.assert(eq(r.chargeable, 2), "T4 chargeable should be actual when greater");

    r = useBoxCalculations({ L: 10, W: 10, H: 10, units: "cm", boardMM: 0, divisor: 0, actualWeight: 0 });
    console.assert(Number.isFinite(r.volWeight) && r.volWeight === 1000, "T5 divisor guard uses >=1");

    r = useBoxCalculations({ L: 10, W: 10, H: 10, units: "weird", boardMM: 0, divisor: 5000, actualWeight: 0 });
    console.assert(eq(r.volWeight, 1000 / 5000), "T6 units fallback to cm");

    r = useBoxCalculations({ L: 30, W: 22, H: 10, units: "cm", boardMM: 0, divisor: 8000, actualWeight: 0 });
    console.assert(eq(r.volWeight, 6600/8000), "T7a divisor 8000");
    r = useBoxCalculations({ L: 30, W: 22, H: 10, units: "cm", boardMM: 0, divisor: 9000, actualWeight: 0 });
    console.assert(eq(r.volWeight, 6600/9000), "T7b divisor 9000");

    // Sheet pricing checks
    let s = computeSheetCNY({ weightKg: 0.99, battery: false, country: 'United States', table: YUNEXPRESS_US_TABLE });
    // No Battery 0.99 → bracket 0.7–2.0, fee 91/kg, item 13 → 91*0.99 + 13 = 103.09
    console.assert(eq(s.totalCNY, 91*0.99 + 13), "T8 sheet no-battery 0.99kg");

    s = computeSheetCNY({ weightKg: 0.05, battery: true, country: 'United States', table: YUNEXPRESS_US_TABLE });
    // Battery 0.05 → bracket 0–0.1, min 0.03 → bill 0.05, fee 108/kg + 24 item → 29.4
    console.assert(eq(s.totalCNY, 108*0.05 + 24), "T9 sheet battery 0.05kg");

    // Manual mode basic
    const m = computeManualCNY({ weightKg: 1.01, rates: { perKgCNY: 50, minChargeCNY: 0 } });
    console.assert(m.totalCNY === 100, "T10 manual ceil per-kg");

    // FX
    const usd = (91*0.99+13)/7.2; // just sanity
    console.assert(eq(usd, (103.09)/7.2, 1e-6), "T11 FX sanity");

    window.__PACKAGING_TESTED__ = true;
    console.log("PackagingBoxDesigner inline tests: ✔️ completed");
  }, []);
  return null;
}
